...
#    vital = pd.read_parquet(datafolder+site+'/p0_vital_'+site+'.parquet')
    ...
    vital = (pd.merge(vital, newdfX, left_on=['PATID', 'ENCOUNTERID'], right_on=['PATID', 'ENCOUNTERID'], how='left').astype({'dummy': 'boolean'}).fillna({'dummy': False}).astype({'dummy': bool}) >> mask(X.dummy) >> select(~X.dummy) >> mutate(FUTURE=X.SINCE_ADMIT-X.DAYS_SINCE_ADMIT)).reset_index(drop=True)
    ...
    vital = vital[['PATID', 'ENCOUNTERID', 'DAYS_SINCE_ADMIT', 'HT', 'WT', 'DIASTOLIC', 'SYSTOLIC', 'ORIGINAL_BMI']]
...
def demo(configs_variables, year):
    ...
    demo = (pd.merge(demo, newdfX, left_on=['PATID', 'ENCOUNTERID'], right_on=['PATID', 'ENCOUNTERID'], how='left').astype({'dummy': 'boolean'}).fillna({'dummy': False}).astype({'dummy': bool}) >> mask(X.dummy) >> select(~X.dummy)).reset_index(drop=True)
    ...
    enc.fit(demo[var])
    ####getfeatures
    demo_onehot_cat = pd.DataFrame(enc.transform(demo[var]).toarray(), columns=enc.get_feature_names_out(var)).astype('bool')
    demo_one = pd.concat([demo[['PATID', 'ENCOUNTERID', 'AGE']].reset_index(), demo_onehot_cat], axis=1).drop('index',axis=1)    
    demo_one = demo_one.drop_duplicates()
    #Save table
...
def dx(configs_variables, year):
    ...
    dx['DX_TYPE'] = dx['DX_TYPE'].str.replace(r'\.0','')
    ...
    newdfX = pd.read_pickle(datafolder+site+'/onset_'+site+'_'+str(year)+'.pkl')
    newdfX = newdfX >> select('PATID', 'ENCOUNTERID', 'SINCE_ADMIT') >> mutate(dummy = True)
    dx = (pd.merge(dx, newdfX, left_on=['PATID', 'ENCOUNTERID'], right_on=['PATID', 'ENCOUNTERID'], how='left').astype({'dummy': 'boolean'}).fillna({'dummy': False}).astype({'dummy': bool}) >> mask(X.dummy) >> select(~X.dummy)).reset_index(drop=True)
...
#    dx_t = (dx_t >> drop('DAYS_SINCE_ADMIT') >> mutate(dummy = True) >> distinct()).pivot(index=['PATID', 'ENCOUNTERID'], columns='DX', values='dummy').fillna(False).reset_index()
    dx_t = (dx_t >> drop('DAYS_SINCE_ADMIT') >> mutate(dummy = True) >> distinct())[['PATID', 'ENCOUNTERID', 'DX', 'dummy']].drop_duplicates().pivot(index=['PATID', 'ENCOUNTERID'], columns='DX', values='dummy').astype('boolean').fillna(False).astype(bool).reset_index()
...
def px(configs_variables, year):
    ...
    newdfX = pd.read_pickle(datafolder+site+'/onset_'+site+'_'+str(year)+'.pkl')
    newdfX = newdfX >> select('PATID', 'ENCOUNTERID', 'SINCE_ADMIT') >> mutate(dummy = True)
    px = (pd.merge(px, newdfX, left_on=['PATID', 'ENCOUNTERID'], right_on=['PATID', 'ENCOUNTERID'], how='left').astype({'dummy': 'boolean'}).fillna({'dummy': False}).astype({'dummy': bool}) >> mask(X.dummy) >> select(~X.dummy) >> mutate(FUTURE=X.SINCE_ADMIT-X.DAYS_SINCE_ADMIT)).reset_index(drop=True)
    ...
    # Fillna if no record
    px_t = (px >> drop('DAYS_SINCE_ADMIT') >> mutate(dummy = True) >> distinct())[['PATID', 'ENCOUNTERID', 'PX', 'dummy']].drop_duplicates().pivot(index=['PATID', 'ENCOUNTERID'], columns='PX', values='dummy').astype('boolean').fillna(False).astype(bool).reset_index()
...
def lab(configs_variables, year):
    ...
    newdfX = pd.read_pickle(datafolder+site+'/onset_'+site+'_'+str(year)+'.pkl')
    newdfX = newdfX >> select('PATID', 'ENCOUNTERID', 'SINCE_ADMIT') >> mutate(dummy = True)
    lab = (pd.merge(lab, newdfX, left_on=['PATID', 'ENCOUNTERID'], right_on=['PATID', 'ENCOUNTERID'], how='left').astype({'dummy': 'boolean'}).fillna({'dummy': False}).astype({'dummy': bool}) >> mask(X.dummy) >> select(~X.dummy) >> mutate(FUTURE=X.SINCE_ADMIT-X.DAYS_SINCE_ADMIT)).reset_index(drop=True)
    ...
    if lab_cat.empty:
        ...
        labcat_t = labcat_t.pivot(index=['PATID', 'ENCOUNTERID'], columns='LAB_LOINC', values='dummy').astype('boolean').fillna(False).astype(bool).reset_index()        
        #Save table    
        labcat_t.to_pickle(datafolder+site+'/labcat_'+site+'_'+str(year)+'.pkl') 
...
def amed(configs_variables, year):
    ...
    newdfX = pd.read_pickle(datafolder+site+'/onset_'+site+'_'+str(year)+'.pkl')
    newdfX = newdfX >> select('PATID', 'ENCOUNTERID', 'SINCE_ADMIT') >> mutate(dummy = True)
    amed = (pd.merge(amed, newdfX, left_on=['PATID', 'ENCOUNTERID'], right_on=['PATID', 'ENCOUNTERID'], how='left').astype({'dummy': 'boolean'}).fillna({'dummy': False}).astype({'dummy': bool}) >> mask(X.dummy) >> select(~X.dummy) >> mutate(FUTURE=X.SINCE_ADMIT-X.DAYS_SINCE_ADMIT)).reset_index(drop=True)
...
#     amed = pd.concat([amed_rx, amed_ndc], axis=0, ignore_index=True)   
    ...
    # Fillna if no record    
    amed_t = (amed >> drop('DAYS_SINCE_ADMIT') >> mutate(dummy = True) >> distinct()).pivot(index=['PATID', 'ENCOUNTERID'], columns='MEDADMIN_CODE', values='dummy').astype('boolean').fillna(False).astype(bool).reset_index()
...
def unify_lab(configs_variables):
    ...
    UCUMunitX = pd.read_csv('/home/xu.zijian/code/AKI_CDM_PY/UCUMunitX.csv')
    UCUMunitX = UCUMunitX[UCUMunitX['factor_final'].notna()]
    UCUMunitX = UCUMunitX.drop('Unnamed: 0',axis=1).drop_duplicates()
    
    
    local_custom_convert = pd.read_csv('/home/xu.zijian/code/202/local_custom_convert.csv')
    local_custom_convert = local_custom_convert.drop('Unnamed: 0',axis=1).drop_duplicates()
    
    UCUMqualX = pd.read_csv('/home/xu.zijian/code/AKI_CDM_PY/UCUMqualX.csv')
