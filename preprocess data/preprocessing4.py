...
def bt_postprocess(site, year, newdf):
    ...
    print('Finishing on site '+site+":"+str(year)) 
...
def combinebtpos(configs_variables, yearX=3000, n_jobs=1):
    ...
    onset = pd.read_parquet(datafolder+configs_variables['site']+'/p0_onset_'+configs_variables['site']+'.parquet')
    years = list(pd.to_datetime(onset['ADMIT_DATE']).dt.year.unique())    
    bt_list = list()
...
#    bt_all = bt_ckd(site, yearX, bt_all)
    ...
    print("END for finishing")
    #Add back correct daat types of columns
    bt_all = correct_dtypes(bt_all, site, stg)
    
    print("END for correct_dtypes")
    # Saveguard
    bt_all = bt_all.drop_duplicates()
    print(bt_all)
    bt_all.to_pickle(datafolder+site+'/bt3pos_'+site+'_'+stg+'_3000_COVID.pkl')
